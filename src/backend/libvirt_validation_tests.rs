...
mod cloud_init_validation_tests {
    ...
    async fn test_exponential_backoff_ssh() {
        ...
        let start = Instant::now();
        let _result = backend.wait_for_ssh(
    ...
    async fn test_wait_for_ip_private_helper() {
        ...
        let result = backend.wait_for_ip(
            "nonexistent-vm",
...
mod integration_tests {
    ...
    async fn test_real_vm_cloud_init_validation() {
        ...
        let node = backend.create_desktop_vm(
            ...
            2,
            10,
        ).await.expect("VM creation failed");
    ...
    async fn test_real_vm_create_ready() {
        ...
        ).await.expect("VM creation with validation failed");
        
        println!("VM fully ready: {} @ {}", node.name, node.ip_address);
        
        // Verify SSH actually works
        let ssh_result = SshClient::connect(
            &node.ip_address,
            22,
            "testuser",
            "testpass123",
        ...
        assert!(ssh_result.is_ok(), "SSH should work immediately after create_ready");
    }
}
