...
#[cfg(test)]
mod tests {
    use super::parse_vnc_from_domain_xml;

    #[test]
    fn parse_vnc_from_domain_xml_detects_port_single_quotes() {
        let xml = r#"<graphics type='vnc' port='5905' listen='0.0.0.0'/>"#;
        assert_eq!(
            parse_vnc_from_domain_xml(xml),
            Some("localhost:5905".to_string())
        );
    }

    #[test]
    fn parse_vnc_from_domain_xml_detects_port_double_quotes() {
        let xml = r#"<graphics type="vnc" port="5910" listen="127.0.0.1"/>"#;
        assert_eq!(
            parse_vnc_from_domain_xml(xml),
            Some("localhost:5910".to_string())
        );
    }

    #[test]
    fn parse_vnc_from_domain_xml_line_scan_fallback() {
        let xml = "  <graphics type='vnc' port='5920'/>\n";
        assert_eq!(
            parse_vnc_from_domain_xml(xml),
            Some("localhost:5920".to_string())
        );
    }

    #[test]
    fn parse_vnc_from_domain_xml_ignores_non_positive_port() {
        let xml = r#"<graphics type='vnc' port='0'/>"#;
        assert_eq!(parse_vnc_from_domain_xml(xml), None);
    }

    #[test]
    fn parse_vnc_from_domain_xml_returns_none_without_vnc() {
        let xml = r#"<graphics type='spice' port='5900'/>"#;
        assert_eq!(parse_vnc_from_domain_xml(xml), None);
    }
}
