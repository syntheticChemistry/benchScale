...
//! Legacy TOML-based configuration (deprecated).
//!
//! New code should use [`super::BenchScaleConfig`] and the typed submodules in this
//! `config` crate. This module preserves the older `Config` graph and environment
...
#[cfg(test)]
#[allow(unsafe_code)] // Centralized in `EnvGuard` / `BenchscaleEnvSnapshot` with `SAFETY:` notes (Rust 2024 `set_var`/`remove_var`).
mod tests {
    use super::*;
    use std::sync::Mutex;

    /// Serializes env mutation across tests (`cargo test` runs tests in parallel by default).
    static CONFIG_ENV_TEST_LOCK: Mutex<()> = Mutex::new(());

    /// Temporarily sets an environment variable and restores the previous value (or unsets) on drop.
    /// Only for tests; `set_var` / `remove_var` are `unsafe` in Rust 2024 as they are not thread-safe.
    struct EnvGuard {
        key: String,
    ...
    impl EnvGuard {
        fn set(key: &str, val: &str) -> Self {
            ...
            // SAFETY: Only used from single-threaded `cargo test` (Rust test harness runs tests with a mutex unless `--test-threads` > 1; callers must not share env across threads).
            unsafe {
    ...
    impl Drop for EnvGuard {
        fn drop(&mut self) {
            // SAFETY: Same as `EnvGuard::set` — test-only, single-threaded env mutation.
            unsafe {
                match &self.original {
    ...
    /// Removes all `BENCHSCALE_*` variables for the duration of the test and restores them on drop.
    struct BenchscaleEnvSnapshot {
        previous: Vec<(String, String)>,
    }

    impl BenchscaleEnvSnapshot {
        fn new() -> Self {
            let previous: Vec<(String, String)> = std::env::vars()
                .filter(|(k, _)| k.starts_with("BENCHSCALE_"))
                .collect();
            for (key, _) in &previous {
                // SAFETY: Test-only; restores previous snapshot in `Drop`.
                unsafe {
    ...
    impl Drop for BenchscaleEnvSnapshot {
        fn drop(&mut self) {
            for (key, val) in self.previous.drain(..) {
                // SAFETY: Test-only env restoration after isolated reads in `Config` tests.
                unsafe {
                    std::env::set_var(&key, val);
                }
            }
    ...
    fn test_default_config() {
        let _lock = CONFIG_ENV_TEST_LOCK.lock().unwrap_or_else(|e| e.into_inner());
    ...
    fn test_env_var_libvirt_uri() {
        ...
        let _uri = EnvGuard::set("BENCHSCALE_LIBVIRT_URI", "qemu+ssh://host/system");
